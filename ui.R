...
winner <- read.csv(paste0(base_uri, resource_winner), stringsAsFactors = F)
...
filter_country <- setNames(
                    as.list(as.character(filter_country$bornCountryCode)),
...
shinyUI(navbarPage(
    ...
    fluidRow(
      tags$div(
        ...
        tags$p(id = "map", "The visual above lets users view the distribution of
               laureate winners across a world map and by choice a specific
               country. The visual can also be filtered by occupation and by
               gender. Users can see the uneven distribution of winners being
               born from Western countries as opposed to any other. Looking more
               specifically, a large number of these winners come from America.
               However, only 12 of the roughly estimated 200 laureates are
               women. This visual ultimately encourages the user to explore the
               trends and impact of what qualifies an individual to be Nobel
               Prize winner.")
      )
    )
  ),
  ...
  tabPanel(
    ...
    fluidRow(
      tags$div(
        class = "summary", checked = NA,
        tags$h6(HTML("Welcome to our Nobel Prize Project, a collaboration by
                     </br><b>Jessica, Yixin, Samantha, and Abraham.</b></br>
                     People don't think much about the Nobel Prize and the
                     significance of its winners. We created this for our
                     INFO 201 Final Project in hopes that you may learn such
                     insights. Who knows? Maybe one of these
                     random facts will stick in your brain, and help you win if
